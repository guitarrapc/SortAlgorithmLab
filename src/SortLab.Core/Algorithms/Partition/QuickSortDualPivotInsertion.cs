...
/// <list type="bullet">
/// <item><description><strong>For arrays &lt; 47 elements (but &gt; InsertThreshold):</strong> Use simple method (leftmost and rightmost elements as pivots, ensuring p1 ≤ p2)</description></item>
...
/// <item><description><strong>Insertion Sort Threshold:</strong> This implementation uses a fixed threshold of 16 elements.
/// Java uses 47 for primitive types, with additional thresholds for different algorithm selections.</description></item>
/// <item><description><strong>Adaptive Algorithm Selection:</strong> Java adaptively selects from multiple algorithms (insertion/merge/counting sort).
/// This implementation uses only dual-pivot + insertion sort hybrid.</description></item>
...
/// <item><description><strong>Purpose:</strong> This is an educational implementation focusing on the hybrid approach (dual-pivot + insertion sort)
/// with production-quality pivot selection. Java's implementation includes additional optimizations for sorted runs and value range analysis.</description></item>
/// </list>
/// </remarks>
public static class QuickSortDualPivotInsertion
{
    // Threshold for switching from QuickSort to InsertionSort
    // Arrays of size <= InsertThreshold are sorted with InsertionSort
    //
    // Why 16 (not 47 like Java)?
    // - Java uses 47 for primitive arrays (int[], etc.) with direct memory access
    // - C# generic types incur virtual method call overhead (IComparable<T>.CompareTo)
    // - Empirical testing on .NET shows 16 provides good balance:
    //   * Below 16: QuickSort overhead dominates
    //   * Above 16: InsertionSort's O(n²) becomes costly with generic constraints
    // - Educational clarity: 16 = 2⁴ is easy to understand and reason about
    //
    // Alternative values to consider (with benchmarking):
    // - 32: Balance between Java's 47 and current 16
    // - 47: Java-compatible, but may not be optimal for C# generics
    ...
    internal static void SortCore<T>(SortSpan<T> s, int left, int right) where T : IComparable<T>
    {
        ...
        if (length <= InsertThreshold)
        {
            InsertionSort.SortCore(s, left, right + 1);
            return;
        }
        
        // For medium-sized arrays, use simple pivot selection (left and right)
