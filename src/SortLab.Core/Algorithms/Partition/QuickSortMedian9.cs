...
public static class QuickSortMedian9
{
    ...
    /// </summary>
    private static T MedianOf9Value<T>(SortSpan<T> s, int low, int high) where T : IComparable<T>
    {
        var m2 = (high - low) / 2;
        var m4 = m2 / 2;
        var m8 = m4 / 2;
        ...
        // Return median of the three medians (using value comparison)
        if (median1.CompareTo(median2) > 0)
        {
            if (median2.CompareTo(median3) > 0)
            {
                return median2; // median1 > median2 > median3
            }
            else
            {
                return median1.CompareTo(median3) > 0 ? median3 : median1;
            }
        }
        else // median1 <= median2
        {
            if (median2.CompareTo(median3) > 0)
            {
                return median1.CompareTo(median3) > 0 ? median1 : median3;
            }
            else
            {
                return median2; // median1 <= median2 <= median3
